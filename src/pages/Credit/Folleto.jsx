...
export function Folleto({ modePay, changeModePay, TABLA, TABLASEMANAL }) {
  // Días y créditos que queremos mostrar
  const diasFiltrados = [20, 25, 35, 50];
  const creditosFiltrados = [100000, 200000,400000, 500000, 800000, 1000000, 1500000];

  // Filtrar TABLA solo con los créditos y días seleccionados
  const tablaFiltrada = TABLA
    .filter((c) => creditosFiltrados.includes(c.credito))
    .map((c) => ({
      ...c,
      dias: c.dias.filter((d) => diasFiltrados.includes(d.dias)),
    }));

  // Para el semanal (si aplica)
  const tablaSemanalFiltrada = TABLASEMANAL
    .filter((c) => creditosFiltrados.includes(c.credito))
    .map((c) => ({
      ...c,
      semanas: c.semanas.filter((s) => diasFiltrados.includes(s.semanasTot)),
    }));

  return (
    <div className="w-[100%] h-[100%] text-black md:w-[680px] nd:h-[1000px] m-auto">
      <div className="flex w-[99%] sm:w-[90%] mx-auto">
      ...
      <div
        ...
        className="px-2 flex flex-col sm:text-xl mt-5 font-semibold"
      >
        <div className="flex w-full justify-between items-center mb-2">
          <h1 className="text-green-900 ml-1 sm:text-2xl font-bold">CRÉDITO</h1>

          <div className="flex justify-between w-[81%]">
            {tablaFiltrada[0].dias.map((dia) => (
              <div className="flex w-full justify-center" key={dia.dias}>
        ...
        {tablaFiltrada.map((creditData) => (
          <div key={creditData.credito}>
      ...
      >
        <div className="flex w-full justify-between items-center mb-2">
          ...
          <div className="flex justify-between w-[81%]">
            {tablaSemanalFiltrada[0].semanas.map((semana) => (
              <div className="flex w-full justify-center" key={semana.semanasTot}>
        ...
        {tablaSemanalFiltrada.map((creditData) => (
          <div key={creditData.credito}>
            <div className="h-auto flex w-full justify-between text-black">
              <h1 className="sm:text-2xl text-blue-950 mr-3 ">
                ${creditData.creditoStr}
              ...
              <div className="flex w-[81%] justify-between">
                {creditData.semanas.map((semana) => (
                  <div
                    className="flex flex-col sm:flex-row w-full items-center justify-center"
                    key={semana.semanasTot}
                  >
                    <h1 className="text-start mr-1 sm:text-2xl text-green-950">
      ...
      <div className="w-[70%] mx-auto mt-7">
        <img className="w-[15%]" src="/creditsmedia/wtsappimg.png" alt="" />
